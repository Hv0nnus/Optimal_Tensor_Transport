...
def gromov_wasserstein_barycenter(costs: Dict, p_s: Dict, p_center: np.ndarray,
    ...
    while relative_error > ot_hyperpara['cost_bound'] and i < ot_hyperpara['max_iter']:
        ...
        if isinstance(barycenter0, np.ndarray):
            error_barycenter0 = barycenter0
        else:
            error_barycenter0 = barycenter0.toarray()

        error_barycenter = barycenter

        if np.sum(np.abs(error_barycenter0)) != 0:
            relative_error = np.sum(np.abs(error_barycenter - error_barycenter0)) / np.sum(np.abs(error_barycenter0))
        else:
            relative_error = np.inf
        i += 1
